import { Type, TypedArrayConstructor, TypedArray, EncodeInput, DecodeInput } from "./../DataManager"
...
export class StringType implements Type<string> {
	...
	static toTypedArray(value: string, constructor: TypedArrayConstructor = Uint16Array) {
		const result = new constructor(value.length)
		for (let i = 0; i < value.length; i++) {
			const code = value.charCodeAt(i)
			result[i] = code
			if (result[i] !== code)
				throw Error('Invalid char code: ' + code)
		}
		return result
	}
