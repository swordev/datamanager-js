...
export interface TypedArrayConstructor<TValue = number | bigint> {
  ...
  new (arrayOrArrayBuffer: ArrayLike<TValue> | ArrayBufferLike): TypedArray<
    TValue,
    Function
  >;
  new (
    ...
    byteOffset: number,
    length?: number
  ): TypedArray<TValue, Function>;
  readonly BYTES_PER_ELEMENT: number;
  readonly prototype: TypedArray<TValue, Function>;
}

export interface TypedArray<TValue = number | bigint, TConstructor = Function>
  extends ArrayLike<TValue> {
...
export interface StrictTypedArray<TValue = number | bigint>
  extends TypedArray<TValue, TypedArrayConstructor<TValue>> {}
...
export class DataManager {
  ...
  addTypeConfig(...configs: TypeConfig[]) {
    for (const config of configs) Object.assign(this.typeConfig, config);
  }

  encodeHeader(
    data: [number, TypedArray][],
    extraData: [number, TypedArray][]
  ): TypedArray[] {
  ...
  encodeValue(
    value: any,
    extraEncodedValues: [number, TypedArray][] = [],
    startIndex = 0
  ): [number, TypedArray] {
  ...
  decodeValue(buffer: ArrayBuffer, header: Header, index: number): any {
    ...
    if (DataManager.isArrayTyped(type)) {
      ...
      return type.decode({
        ...
        toTypedArray: (constructor) =>
          new constructor(buffer, header.offsets[index], header.lengths[index]),
  ...
  encode(input: any[]): ArrayBuffer {
    ...
    const encodedValues = input.map((value) =>
      this.encodeValue(value, extraEncodedValues, input.length)
    );

    const data = this.encodeHeader(encodedValues, extraEncodedValues)
  ...
  static offsets(
    data: { BYTES_PER_ELEMENT: number; length: number }[],
    byteLength = 0
  ) {
  ...
  static encode(data: TypedArray[]) {
    ...
    data.forEach((data, index) => {
      const writer = new (data as StrictTypedArray).constructor(
        buffer,
  ...
  static decode<T = number | bigint>(
    ...
    const result = data.map(
      (item, index) => new item[0](buffer, offsets.result[index], item[1])
    );
