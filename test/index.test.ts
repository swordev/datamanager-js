...
describe("DataManager.isArrayTyped", () => {
  ...
  it("error", () => {
    expect(
      DataManager.isArrayTyped(new DataView(new ArrayBuffer(1)))
    ).toBeFalsy();
...
describe("DataManager.offsets", () => {
  ...
  it("with byteLength", () => {
    ...
    ).toMatchObject({ result: [3, 8], byteLength: 48 });
    ...
    ).toMatchObject({ result: [4, 16], byteLength: 56 });
  });
});
...
describe("StringType.toTypedArray", () => {
  it("success", () => {
    expect(Types.StringType.toTypedArray("hello world ♥")).toMatchObject(
      new Uint16Array([
        104, 101, 108, 108, 111, 32, 119, 111, 114, 108, 100, 32, 9829,
      ])
    );
...
describe("Datamanager.prototype.encode/decode", () => {
  ...
  it("custom date type", () => {
    ...
    const data = [new Date(1565858125372), { date: new Date(1565858125373) }];
  ...
  it("custom string type", () => {
    ...
    dm.addTypeConfig({
      100: {
        ...
        decode: (data) =>
          Types.StringType.fromTypedArray(data.toTypedArray(Uint8Array)),
      },
    });

    const data = [JSON.stringify({ title: "hello world" })];
